...
public record PeriodComparison(String period, String currentLabel, String previousLabel,
    ...
    public static PeriodComparison previous(String period, LocalDate today, ZoneId zone, String teamId, String userId) {
        return switch (period) {
            case "day" -> day(today, zone, teamId, userId);
            case "week" -> week(today, zone, teamId, userId);
            case "month" -> month(today, zone, teamId, userId);
            default -> throw new BadRequestException("period must be one of day, week, or month");
        };
    }

    private static PeriodComparison day(LocalDate today, ZoneId zone, String teamId, String userId) {
        ReportQuery current = new ReportQuery("day", today, today, zone, teamId, userId);
        LocalDate yesterday = today.minusDays(1);
        ReportQuery previous = new ReportQuery("day", yesterday, yesterday, zone, teamId, userId);
        return new PeriodComparison("day", "Today", "Yesterday", current, previous);
    }

    private static PeriodComparison week(LocalDate today, ZoneId zone, String teamId, String userId) {
        ...
        ReportQuery current = new ReportQuery("week", currentStart, today, zone, teamId, userId);
        ReportQuery previous = new ReportQuery("week", previousStart, previousStart.plusDays(elapsedDays), zone, teamId, userId);
        return new PeriodComparison("week", "This Week", "Previous Week", current, previous);
    }

    private static PeriodComparison month(LocalDate today, ZoneId zone, String teamId, String userId) {
        YearMonth currentMonth = YearMonth.from(today);
        YearMonth previousMonth = currentMonth.minusMonths(1);
        int previousDay = Math.min(today.getDayOfMonth(), previousMonth.lengthOfMonth());
        ReportQuery current = new ReportQuery("month", currentMonth.atDay(1), today, zone, teamId, userId);
