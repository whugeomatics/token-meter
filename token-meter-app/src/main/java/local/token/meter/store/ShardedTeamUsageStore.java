...
import local.token.meter.domain.DeviceTokenRecord;
...
public final class ShardedTeamUsageStore implements TeamUsageStore {
    ...
    public ShardedTeamUsageStore(Path rootDir) throws java.io.IOException {
        ...
        this.registry = new SqliteTeamUsageStore(rootDir.resolve("token-meter-team-registry.sqlite"),
                true, false, true);
    }
    ...
    private Path dbPath(YearMonth shard) {
        return rootDir.resolve("token-meter-team-" + shard + ".sqlite");
    }
}
