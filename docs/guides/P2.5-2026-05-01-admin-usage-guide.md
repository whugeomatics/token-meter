...
export TOKEN_METER_DEVICE_TOKEN="UTF4xtfooc0vQS4xMWgZyzOYWY3YLIAPkoG7o8qSPgc"
...
```text
dist/token-meter-collector-P2.5-2026-05-01/
...
安装脚本默认会从脚本同目录寻找 `token-meter-collector-0.1.0-SNAPSHOT.jar`。如果 teammate 的 Java 不在标准 PATH 中，脚本会优先使用 `JAVA_HOME/bin/java`，也可以显式传 `TOKEN_METER_JAVA`。
