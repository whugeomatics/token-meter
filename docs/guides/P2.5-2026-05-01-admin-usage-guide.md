...
```sh
java -jar agent-dashboard-app/target/agent-dashboard-app-0.1.0-SNAPSHOT.jar \
  --db="$HOME/.agent-dashboard/sqlite" \
...
- 不配置 `--admin-token` 或 `AGENT_DASHBOARD_ADMIN_TOKEN` 时，admin 页面关闭。
- admin token 不会写入数据库、日志、HTML 或 JS。
- SQLite 默认在 `~/.agent-dashboard/sqlite` 下按月分片保存。

也可以用环境变量启动：

```sh
export AGENT_DASHBOARD_ADMIN_TOKEN="replace-with-admin-token"
java -jar agent-dashboard-app/target/agent-dashboard-app-0.1.0-SNAPSHOT.jar \
  --db="$HOME/.agent-dashboard/sqlite"
```
...
```sh
java -jar agent-dashboard-collector/target/agent-dashboard-collector-0.1.0-SNAPSHOT.jar \
  --collect-team \
...
```sh
export AGENT_DASHBOARD_SERVER_URL="http://127.0.0.1:18080"
export AGENT_DASHBOARD_DEVICE_TOKEN="UTF4xtfooc0vQS4xMWgZyzOYWY3YLIAPkoG7o8qSPgc"
export AGENT_DASHBOARD_USER_ID="zhangsan"
export AGENT_DASHBOARD_DEVICE_ID="zhangsan-macbook"
sh scripts/P2.5-2026-05-01-run-collector.sh
```

脚本启动时会先检查 `${AGENT_DASHBOARD_SERVER_URL}/health`。如果 dashboard 实际启动在 `18080`，但 `AGENT_DASHBOARD_SERVER_URL` 仍然配置成 `10080`，collector 会直接提示服务不可达。
...
P2.5 推荐用 mac `launchd` 做周期上报，而不是让 Java collector 常驻。
...
```sh
sh scripts/P2.5-2026-05-01-package-collector.sh
```

把输出目录发给 teammate：

```text
dist/agent-dashboard-collector-P2.5-2026-05-01/
...
```sh
export AGENT_DASHBOARD_SERVER_URL="http://127.0.0.1:18080"
export AGENT_DASHBOARD_DEVICE_TOKEN="teammate-device-token"
export AGENT_DASHBOARD_USER_ID="zhangsan"
export AGENT_DASHBOARD_DEVICE_ID="zhangsan-macbook"

sh P2.5-2026-05-01-install-collector-service.sh
```

`AGENT_DASHBOARD_SERVER_URL` 必须是 teammate 电脑能访问到的 dashboard server 地址。跨机器时不要写 `127.0.0.1`，除非 server 也在 teammate 自己电脑上。

可选配置：

```sh
export AGENT_DASHBOARD_COLLECTOR_INTERVAL_SECONDS=300
export AGENT_DASHBOARD_DAYS=30
export AGENT_DASHBOARD_JAVA="/absolute/path/to/java"
```

安装脚本默认会从脚本同目录寻找 `agent-dashboard-collector-0.1.0-SNAPSHOT.jar`。如果 teammate 的 Java 不在标准 PATH 中，脚本会优先使用 `JAVA_HOME/bin/java`，也可以显式传 `AGENT_DASHBOARD_JAVA`。

如果安装时出现：

```text
Could not kickstart service "local.agent.dashboard.collector": 1: Operation not permitted
```

这通常表示 macOS 拒绝了立即触发，不等于 plist 和配置没有写入。脚本会保留安装结果，服务会按 `StartInterval` 周期运行；也可以在 teammate 终端手动执行：

```sh
launchctl kickstart -k "gui/$(id -u)/local.agent.dashboard.collector"
```

安装后文件位置：

```text
~/Library/LaunchAgents/local.agent.dashboard.collector.plist
~/.agent-dashboard/collector.env
~/.agent-dashboard/logs/collector.out.log
~/.agent-dashboard/logs/collector.err.log
```

安全说明：

- `~/.agent-dashboard/collector.env` 会保存 teammate 的 device token。
- 安装脚本会把该文件权限设为 `600`。
...
```sh
launchctl print "gui/$(id -u)/local.agent.dashboard.collector"
```
...
```sh
sh P2.5-2026-05-01-uninstall-collector-service.sh
```

卸载脚本不会删除 `~/.agent-dashboard/collector.env`。如果要清除本机 token 配置，需要手动删除：

```sh
rm ~/.agent-dashboard/collector.env
```

## 6. 跨机器上报注意事项

当前服务端默认只绑定 `127.0.0.1`，这是 P2.5 MVP 的安全默认值。
...
- 后续阶段增加显式 `--bind-host` 配置，并配套鉴权和网络安全说明。
...
P2.5 只统计 Codex JSONL 中的 `token_count` / `total_token_usage`。
...
如果上报的是更早的数据，切换到 `30D` 或 `This Month`。
