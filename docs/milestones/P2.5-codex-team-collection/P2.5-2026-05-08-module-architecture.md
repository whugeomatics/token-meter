# P2.5 Module Architecture

## 目标

P2.5 在团队上报需求中补齐三 module 架构边界，使 teammate 端分发的是轻量 collector jar，而不是包含 dashboard、SQLite、静态资源和管理接口的完整 app jar。

本文件记录 P2.5 结束后必须长期遵守的 module 约束。后续阶段如果需要调整边界，应先更新设计、contract、tasks 和验收文档，再改代码。

## Module 边界

```text
agent-dashboard-core
...
P2.5 smoke/package 脚本必须检查 collector jar 中没有以下内容：

- `local/agent/dashboard/http/`
- `local/agent/dashboard/report/`
- `local/agent/dashboard/store/`
...
P2.5 的 team 上报是临时拆入需求，因此后续补丁必须把文档补齐后再继续开发：
