...
当前阶段：**P5 - Unified CLI usage metrics review follow-up 待处理**。

P1、P2、P3 和 P4 均已通过实现验证。P5 已有实现和验收记录，但 2026-06-15 P5 review 发现 follow-up，解决前不应视为完全关闭。
...
- Local dashboard API 只查询 SQLite；app 通过本地后台采集循环刷新 Codex 与 Claude Code 数据。
...
- P5 实现已有验证记录，但 review follow-up 仍待处理。
- Review follow-up 覆盖 canonical fallback 不一致和 Local DB 完整路径隐私 contract 冲突。
...
- [P4 配置参考](docs/guides/P4-2026-05-25-configuration-reference.md)
...
配置项和典型使用场景见 [P4 配置参考](docs/guides/P4-2026-05-25-configuration-reference.md)。
